...
def calculateAndDisplayWinner(resultsDict, answersDict):
    ...
    prefix = "1st Place: "
    amount = FIRST_AMOUNT
    tieBreakerScore = answersDict["answer"][(int(answersDict["numGames"][0]) * 2)]
    while localCount < 2:
        ...
        if len(winnersList) > 1:
            print("There is a tie with ", str(highestCorrectPicks[1])," points. Result will be determined based on tie breaker score")
            print("Tiebreaker score: " + str(tieBreakerScore))
            for winner in winnersList:
                for winnerKey in winner:
                    participantTieBreaker = answersDict[winnerKey][(int(answersDict["numGames"][0])* 2)]
            ...
            print(Fore.GREEN + prefix + finalWinner + " wins ", "$" + str(amount), " with a tiebreaker score of " + finalWinnerTB + "!")
            resultsDict.pop(finalWinner)
            localCount = localCount + 1
        elif len(winnersList) == 1:
            for winner in winnersList:
                for winnerKey in winner:
                    print(Fore.GREEN + prefix + winnerKey + " wins ", "$" + str(amount), " with " + str(winner[winnerKey]) + " points!")
...
def runSpellChecker(fileName):
    teamNames = ["cardinals","falcons","panthers","bears","cowboys","lions","pack","rams","vikings",
                 ...
                 "dolphins","pats","jets","steelers","titans","empty"]
    inFile = open(fileName)
    for line in inFile:
        line = line.strip()
        line = line.split(",")
        for x in range(len(line)):
            if line[x].lower() not in teamNames and line[x] != line[-1] and x % 2 != 0:
                print(Fore.RED + "You made a typo when entering " + line[x] + " for " + line[0] + " in " + fileName)
                print(Style.RESET_ALL)
                return False
